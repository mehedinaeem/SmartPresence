...
if __name__ == "__main__":
    ...
    cfg=settings(); recognizer=FaceRecognizer()
    for box in FaceDetector().detect(image):
        face=crop_face(image,box,cfg["FACE_DETECTION"]["margin"],(cfg["FACE_SIZE"]["width"],cfg["FACE_SIZE"]["height"]))
        print({"box":box,"prediction":recognizer.recognize(face)})
